...
Piora's companion compatibility layer contains a modified TypeScript adaptation of a small
...
The adaptation is not an official OpenAI integration. It was rewritten for Piora's local-only,
...
bundled as product assets by Piora; only the attributed, modified TypeScript adaptation is part
of Piora. Compatible visual assets are read from a user's local Codex installation only when requested.
