...
Piora 0.2.0 增加了一个桌面专用的 HarmonyOS NEXT 设备控制预览。它让用户先在可见设备面板中连接和检查自己的测试手机，再把结构化、受限的 UI 操作交给 AI 执行。
...
如果显示“未找到 HDC”，在面板的运行时设置中粘贴 `hdc.exe` 的完整路径并保存，然后重新探测。配置保存在 Piora 桌面数据目录，不写入项目仓库。
