...
提取提示词建议每个完成的助手回复输出最多 8 个气泡、3 组关系信息，服务端不强制数量限制；结果保留分组关系信息，界面统一平铺展示。点击选项把完整短句加入输入框，仍由用户按发送键提交。互斥方案在同一单选组内替换；多选组和不同问题可以组合。不显示推荐标签，不默认选中。提取期间和没有明确选择时不显示任何占位区，测试按钮也不显示“正在提取”文字。
...
客户端请求键包含 session ID、leaf ID、source entry ID、正文、提取模型、系统提示词、语言、协议版本。切换来源、设置或运行状态会终止旧请求；迟到的缓存读取和网络结果也有独立存活检查。409 表示来源尚未就绪或已过时，短暂等待后有限重试；其他失败显示原因和手动重试按钮，设置页的测试提取也展示具体错误。不影响聊天发送。
...
固定 JSON 输出协议追加在用户提取规则之后，解析兼容 Markdown 代码块包裹和额外字段。不再按组数、项数、字数、重复内容、单选至少两项或依据是否逐字匹配拒绝结果。标签和填入文字可互相补缺，缺少分组模式默认多选；缺少依据或推荐标记不影响展示，无可显示文字的条目单独跳过。应用生成唯一标识，重复选项仍可独立操作。只有 JSON 无法解析或缺少 groups 数组时才报告格式读取错误。

分析正文去除 fenced code、引用行和缩进示例；超过 24,000 字符只保留尾部完整段落，无完整段落则不提取。不传图片、思考、工具输出或完整会话历史。选项内容与原文关系由提取模型判断，服务端仅整理渲染所需的数据结构。
...
| 502 | `provider_error` / `invalid_output` | 普通会话和设置预览显示错误 |
| 504 | `timeout` | 用户手动重试 |
