...
状态：Beta 实现完成；阶段 0/1/2/3/4/5/6 已实现并通过自动化验收（2026-09-02）
...
- 窄右栏与最大化工作台共用的响应式文档树界面；支持多范围选择、只读分析，以及设计、计划、代码视图切换。
...
- ArkUI AST、稳定打印器、资源规划器，以及设计节点到文件、符号和资源的来源映射。
- 内容寻址的隔离预览目录、产物清单和文件完整性校验；重复生成可复用字节完全一致的预览，且不会写入用户项目。
- 代码与资源产物预览、现有文件工具跳转、Figma 图片填充/节点渲染的真实 PNG 导出，以及下载失败时确定性 SVG 回退的显式提示。
- 基于项目当前内容的确定性统一补丁、逐文件新增/更新/一致/冲突分类，以及现有 `DiffView`、`FileViewer` 和 Git Review 工作区联动。
- 未托管现有文件、托管后手工修改、已脱管文件、非普通文件、非 UTF-8 文件和超限文件的显式冲突；只有允许覆盖的冲突才能被用户逐项授权。
- 5 分钟有效的一次性 apply token，绑定项目、run revision、补丁哈希和用户批准的覆盖路径；应用前再次校验，陈旧补丁默认失败。
- 项目外持久化的托管/脱管状态，以及多文件同目录暂存、备份换名、事务日志、失败反向回滚和进程重启恢复。
- 项目模块、product、target、bundle/ability 与 DevEco/Hvigor 工具链检测；在项目影子副本中执行可取消、限时、限输出的 `assembleHap`，构建诊断可回溯生成文件和设计节点。
- 复用 Harmony 设备发现与租约系统完成 HAP 安装、启动、稳定截图和 UI 树采集；支持明确选择设备，不抢占已有租约。
- 设计参考图与设备截图的有界对齐、状态栏比例裁切、并排/叠加滑杆/差异图，以及差异区域到 UI 节点和设计节点的关联。
- 基于完整 Normalized Design IR、设计系统元数据和依赖根的版本差异；普通选择保持原范围，原型流程自动纳入递归可达页面，字节相同的生成文件不会进入项目补丁。
- 运行级 SSE 进度、单调 revision、统一取消信号、终态重同步、进程重启后的 `interrupted` 恢复，以及预览、构建和素材缓存回收。
- 中英文响应式工作台、文档/流程/组件/变量搜索、模块/target/product/设备选择、键盘语义和 390px 窄屏布局。
- 多页设计 fixture、适配器/凭据/缓存/API 边界测试、IR/计划/映射/并发测试、ArkUI golden test、两套真实 DevEco 编译 fixture、二进制素材、视觉对齐、补丁/冲突/令牌/回滚/重启恢复、Harmony fake-device 集成测试，以及浏览器级响应式验收。

当前限制：本次开发机没有连接 HarmonyOS 真机，因此设备链路由 fake backend、租约/安装/启动/截图集成测试覆盖；Phone、多尺寸、主题、键盘和系统安全区的物理设备矩阵仍需在有设备的发布验收机执行。首发仍只承诺规格中明确支持的 ArkUI 子集，复杂动画、私有插件节点和无法安全表达的效果继续作为可见问题或回退项。
...
开发阶段使用明确的实验门控制入口，避免半成品进入普通用户工具列表。阶段 1 至阶段 3 必须保留“只生成到预览区”的安全边界；阶段 4 只能通过只读补丁审查、逐项冲突处理和明确确认进入项目写入。
