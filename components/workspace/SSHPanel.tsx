...
export function SSHPanel({ agentSessionId }: { agentSessionId?: string | null }) {
  ...
  const reconnect = async () => { if (!snapshot) return; setError(null); try { const response = await fetch(`/api/ssh/sessions/${snapshot.id}/actions`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "start" }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Reconnect failed"); setSnapshot(data.snapshot); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } };
  ...
  const upload = async (file: File) => { if (!snapshot) return; setUploading(true); try { const response = await fetch(`/api/ssh/sessions/${snapshot.id}/files/upload?path=${encodeURIComponent(`${path.replace(/\/$/, "")}/${file.name}`)}`, { method: "POST", body: file }); if (!response.ok) { const data = await response.json(); throw new Error(data.error || "Upload failed"); } await loadFiles(path); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } finally { setUploading(false); } };
