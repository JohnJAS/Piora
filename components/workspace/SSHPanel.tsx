...
export function SSHPanel() {
  ...
  const loadFiles = async (nextPath: string) => { if (!snapshot) return; try { const response = await fetch(`/api/ssh/sessions/${snapshot.id}/files?path=${encodeURIComponent(nextPath)}`); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Unable to read remote directory"); setPath(data.path); setEntries(data.entries); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } };
  ...
  useEffect(() => { if (snapshot) void loadFiles(path); }, [snapshot]);
