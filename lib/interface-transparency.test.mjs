...
const { parseInterfaceTransparency, hasInterfaceTransparencyPreference, serializeInterfaceTransparency, INTERFACE_TRANSPARENCY_INITIALIZATION_SCRIPT, INTERFACE_TRANSPARENCY_STORAGE_KEY } = await jiti.import("./interface-transparency.ts");
...
test("saved transparency and pre-paint initialization agree on defaults, bounds and malformed data", () => {
  for (const [raw, expected] of [[null, 80], ["invalid", 80], ['{"schemaVersion":2,"transparency":20}', 80], ['{"schemaVersion":1,"transparency":"20"}', 80], ...[-5, 0, 25.6, 80, 100, 120].map(n => [JSON.stringify({ schemaVersion: 1, transparency: n }), Math.min(100, Math.max(0, Math.round(n)))])]) {
    ...
    assert.equal(root.dataset.interfaceTransparencyOverride === "true", hasInterfaceTransparencyPreference(raw));
  }
