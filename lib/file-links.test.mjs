...
test("resolves tool file paths against the session workspace", async () => {
  const { resolveWorkspaceFilePath } = await loadSubject();

  assert.equal(
    resolveWorkspaceFilePath("components/AppShell.tsx", "X:/workspace/repository"),
    "X:/workspace/repository/components/AppShell.tsx",
  );
  assert.equal(
    resolveWorkspaceFilePath("components\\AppShell.tsx", "X:\\workspace\\repository"),
    "X:/workspace/repository/components/AppShell.tsx",
  ...
  assert.equal(
    resolveWorkspaceFilePath("README", "/home/me/project"),
    "/home/me/project/README",
  );
  assert.equal(
    resolveWorkspaceFilePath("../outside.txt", "/home/me/project"),
    null,
  );
  assert.equal(
    resolveWorkspaceFilePath("C:\\repo\\file.ts", "D:/other"),
    "C:/repo/file.ts",
  );
});
