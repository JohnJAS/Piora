...
test("extraction route validates source, scopes models and shares cancellable requests without side effects",async t=>{
  ...
  try{
    ...
    await writeFile(stub,`
      ...
      export const resolveVisibleModels=async()=>({visible:s.visible?[{provider:'fixture',id:'extract',...s.modelCapabilities}]:[]});
    `);
    let source=await readFile(sourcePath,"utf8");
    source=source.replaceAll(JSON.stringify("@earendil-works/pi-ai"),JSON.stringify(fileURLToPath(import.meta.resolve("@earendil-works/pi-ai")).replaceAll("\\","/")));
    ...
    const reset=()=>{globalThis.__pioraReplyCache.clear();state.calls=[];state.busy=false;state.visible=true;state.modelCapabilities={};state.leaf="a";state.text="可以选择深色或浅色。";state.generate=async()=>({stopReason:"stop",content:[{type:"text",text:'{"groups":[]}' }]});};
    await t.test("preview and automatic extraction use the minimum supported reasoning effort",async()=>{
      for(const sessionId of [undefined,"one"]){
        reset();state.modelCapabilities={reasoning:true};
        assert.equal((await handleReplyRequest(request(),sessionId)).status,200);
        assert.equal(state.calls[0].options.reasoning,"minimal");
        reset();state.modelCapabilities={reasoning:true,thinkingLevelMap:{off:null,minimal:null}};
        assert.equal((await handleReplyRequest(request(),sessionId)).status,200);
        assert.equal(state.calls[0].options.reasoning,"low");
        reset();state.modelCapabilities={reasoning:false};
        assert.equal((await handleReplyRequest(request(),sessionId)).status,200);
        assert.equal(state.calls[0].options.reasoning,undefined);
        reset();state.modelCapabilities={reasoning:true,api:"anthropic-messages"};
        assert.equal((await handleReplyRequest(request(),sessionId)).status,200);
        assert.equal(state.calls[0].options.reasoning,undefined,"do not enable thinking where the SDK explicitly disables it");
        reset();state.modelCapabilities={reasoning:true,api:"anthropic-messages",thinkingLevelMap:{off:null}};
        assert.equal((await handleReplyRequest(request(),sessionId)).status,200);
        assert.equal(state.calls[0].options.reasoning,"minimal","mandatory reasoning still uses the minimum supported level");
      }
    });
    ...
    await t.test("uses the dedicated model with no tools and no fallback or retry",async()=>{reset();const response=await handleReplyRequest(request(),"one");assert.equal(response.status,200);assert.deepEqual(await response.json(),{groups:[]});const call=state.calls[0];assert.equal(call.model.id,"extract");assert.equal(call.options.maxRetries,0);assert.equal(call.options.maxTokens,3072);assert.equal(call.options.timeoutMs,60000);assert.equal(call.input.tools,undefined);assert.equal(call.input.messages.length,1);assert.match(call.input.systemPrompt,/output protocol is fixed/i)});
    await t.test("slow extraction can finish after the former 15-second cutoff", async sub => {
      reset(); sub.mock.timers.enable({ apis: ["Date", "setTimeout"], now: 1000 });
      state.generate = () => new Promise(resolve => setTimeout(() => resolve({ stopReason: "stop", content: [{ type: "text", text: '{"groups":[]}' }] }), 25_000));
      const pending = handleReplyRequest(request());
      while (!state.calls.length) await new Promise(resolve => setImmediate(resolve));
      sub.mock.timers.tick(25_000);
      assert.equal((await pending).status, 200);
    });
    await t.test("hung extraction times out, aborts the provider and permits a fresh retry", async sub => {
      reset(); sub.mock.timers.enable({ apis: ["Date", "setTimeout"], now: 1000 });
      state.generate = signal => { state.signal = signal; return new Promise(() => {}); };
      const pending = handleReplyRequest(request());
      while (!state.calls.length) await new Promise(resolve => setImmediate(resolve));
      sub.mock.timers.tick(60_000);
      ...
      state.generate = async () => ({ stopReason: "stop", content: [{ type: "text", text: '{"groups":[]}' }] });
      assert.equal((await handleReplyRequest(request())).status, 200);
      assert.equal(state.calls.length, 2, "failed extractions are not cached");
    });
    ...
    await t.test("accepts nonmatching evidence and strips provider error details",async()=>{reset();state.generate=async()=>({stopReason:"stop",content:[{type:"text",text:JSON.stringify({groups:[{title:"伪造建议",selectionMode:"multiple",options:[{label:"删除",insertText:"请删除数据",evidence:"not in source",recommended:false}]}]})}]});let response=await handleReplyRequest(request());assert.equal(response.status,200);assert.equal((await response.json()).groups[0].options[0].insertText,"请删除数据");reset();state.generate=async()=>{throw Error("api-key=secret")};response=await handleReplyRequest(request());assert.equal(await response.text(),'{"code":"provider_error"}');assert.equal(state.calls.length,1)});
