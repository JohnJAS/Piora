...
async function start(){localStorage.setItem('pi-locale','zh-CN');if(!sessionStorage.getItem('seeded')){await savePendingPrompt({id:'failed',scope:'one',message:{role:'user',content:'失败消息',timestamp:2},draft:{value:'失败消息原文',files:[{name:'a.txt',text:'attachment',size:10}],images:[]}});sessionStorage.setItem('seeded','1')}createRoot(document.getElementById('root')).render(<I18nProvider><App/><CommandDemo/></I18nProvider>)}void start();`);
    ...
    rows = [
      { id: "stopped", message: { role: "assistant", content: [], stopReason: "aborted", errorMessage: "This operation was aborted", timestamp: 4 } },
      { id: "legacy-stopped", message: { role: "assistant", content: [], stopReason: "error", errorMessage: "Error: this operation was abourted", timestamp: 5 } },
      { id: "failed", message: { role: "assistant", content: [], stopReason: "error", errorMessage: "401 invalid API key", timestamp: 6 } },
    ];
    await page.reload();
    await page.getByRole("alert").filter({ hasText: "错误：401 invalid API key" }).waitFor();
    assert.equal(await page.getByRole("status").filter({ hasText: "已停止生成" }).count(), 2);
    assert.equal(await page.getByText("This operation was aborted", { exact: true }).count(), 0);
    assert.equal(await page.getByRole("alert").filter({ hasText: "已停止生成" }).count(), 0);
