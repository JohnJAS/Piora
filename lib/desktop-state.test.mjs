...
const {
  hasShownXiaoyiStartup, markXiaoyiStartupShown, writeLastLaunchedVersion,
...
test("XiaoYi first-use marker is independent of Piora history and survives version upgrades", async t => {
  const root = await mkdtemp(path.join(tmpdir(), "piora-xiaoyi-state-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  assert.equal(hasShownXiaoyiStartup(root, logger), false);
  writeLastLaunchedVersion(root, "0.4.41-beta.57", logger);
  assert.equal(hasShownXiaoyiStartup(root, logger), false);
  assert.equal(markXiaoyiStartupShown(root, logger), true);
  writeLastLaunchedVersion(root, "0.4.41-beta.58", logger);
  assert.equal(hasShownXiaoyiStartup(root, logger), true);
  assert.equal(JSON.parse(await readFile(path.join(root, "desktop-state.json"), "utf8")).lastLaunchedVersion, "0.4.41-beta.58");
});
