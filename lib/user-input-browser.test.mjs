...
test("question card countdown dismisses unanswered drafts and synchronizes request lifecycle", { timeout: 120000 }, async () => {
  ...
  try {
    ...
    assert.ok(initialRemaining >= 59 && initialRemaining <= 60, `expected 59–60 seconds, received ${initialRemaining}`);
    ...
    assert.ok(reconnectRemaining >= 39 && reconnectRemaining <= 40, `expected 39–40 seconds, received ${reconnectRemaining}`);
