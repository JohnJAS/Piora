...
test("folders persist parent links and reject cycles or deletion with children", () => isolated((env) => {
  ...
  transfer.updateTransferItem(child.id, { trash: true }, env);
  assert.ok(transfer.readTransferItems(env).find((item) => item.id === child.id).deletedAt);
  transfer.updateTransferItem(child.id, { restore: true }, env);
...
test("transfer trash keeps recoverable content until permanent deletion", () => isolated((env) => {
  const note = transfer.addTransferItem({ title: "recover", content: "keep", language: "markdown" }, env);
  transfer.updateTransferItem(note.id, { trash: true }, env);
  let stored = transfer.readTransferItems(env).find((item) => item.id === note.id);
  assert.ok(stored.deletedAt); assert.equal(stored.content, "keep");
  transfer.updateTransferItem(note.id, { restore: true }, env);
  stored = transfer.readTransferItems(env).find((item) => item.id === note.id);
  assert.equal(stored.deletedAt, undefined);
  transfer.updateTransferItem(note.id, { trash: true }, env);
  transfer.updateTransferItem(note.id, { remove: true }, env);
  assert.equal(transfer.readTransferItems(env).some((item) => item.id === note.id), false);
}));

test("restoring a nested item restores its folder path and prevents orphan deletion", () => isolated((env) => {
  const folder = transfer.addTransferItem({ title: "Folder", content: "", kind: "folder" }, env);
  const note = transfer.addTransferItem({ title: "Nested", content: "keep", language: "markdown", parentId: folder.id }, env);
  transfer.updateTransferItem(note.id, { trash: true }, env);
  transfer.updateTransferItem(folder.id, { trash: true }, env);
  assert.throws(() => transfer.updateTransferItem(folder.id, { remove: true }, env), /还有内容/);
  transfer.updateTransferItem(note.id, { restore: true }, env);
  const restored = transfer.readTransferItems(env);
  assert.equal(restored.find((item) => item.id === note.id).deletedAt, undefined);
  assert.equal(restored.find((item) => item.id === folder.id).deletedAt, undefined);
  assert.equal(restored.find((item) => item.id === note.id).parentId, folder.id);
}));
