...
test("terminal session keeps shell state and streams output across commands", async (t) => {
  ...
  t.after(() => {
    terminal.dispose();
    // Windows may retain the shell's cwd handle briefly after process.kill().
    // Let rimraf retry instead of turning a successful terminal test into an
    // intermittent EBUSY cleanup failure on CI runners.
