...
test("cancellation is a visible neutral status while real provider errors remain errors", async () => {
  const { isAssistantAborted, hasAssistantNotice } = await loadSubject();
  assert.equal(isAssistantAborted({ ...assistant([]), stopReason: "aborted" }), true);
  for (const errorMessage of ["This operation was aborted", "Error: This operation was abourted", "AbortError: The operation was aborted.", "Request was aborted."]) {
    const message = { ...assistant([]), stopReason: "error", errorMessage };
    assert.equal(isAssistantAborted(message), true);
    assert.equal(hasAssistantNotice(message), true);
    assert.equal(isAssistantAborted(message, { isStreaming: true }), false);
  }
  for (const errorMessage of ["Upstream aborted connection: HTTP 502", "Request timed out", "Invalid API key"]) assert.equal(isAssistantAborted({ ...assistant([]), stopReason: "error", errorMessage }), false);
});
