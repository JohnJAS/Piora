...
test("source license inventory is deterministic and --check detects stale output", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "pi-gui-license-source-"));
...
test("source license inventory fails closed for an unreviewed runtime package", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "pi-gui-license-unreviewed-"));
  t.after(() => rm(root, { recursive: true, force: true }));
...
test("packaged bundle inventories exact package copies and deduplicates complete license texts", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "pi-gui-license-package-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  ...
  await writeFile(join(webRoot, "package.json"), '{"name":"pigui","version":"9.8.7"}\n', "utf8");
  ...
  assert.equal(manifest.schema, "pi-gui-third-party-packages-v3");
  ...
  assert.equal(
    sbom.components.filter((entry) => (
      entry.properties.some((property) => (
        property.name === "piGUI:evidenceScope" && property.value === "runtime-source-closure"
      ))
    )).length,
    4,
  );
...
test("reviewed postinstall replacement records only the final brace-expansion entity", async (t) => {
  const sourceRoot = await mkdtemp(join(tmpdir(), "pi-gui-license-replacement-"));
...
test("version-scoped remark-math license fallback is complete and tamper evident", async (t) => {
  const sourceRoot = await mkdtemp(join(tmpdir(), "pi-gui-license-fallback-"));
  t.after(() => rm(sourceRoot, { recursive: true, force: true }));
  ...
  assert.equal(
    sbom.components.filter((component) => component.properties.some((property) => (
      property.name === "piGUI:reviewedLicenseFallback"
    ))).length,
    2,
  );
  ...
  const fixtures = [
    {
      ...
      sourceSha256: "e7e09d5e6237a7131232be0080ee783d90c1106229fd52eb2676f3a32e208ff6",
    ...
    },
    {
      ...
      sourceSha256: "4cb7c62d2d59cb04acc94d58085b28d420f5c0359f0cf013b889825947cfdb24",
