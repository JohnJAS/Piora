...
test("Fake Agents automatically progress planning, task execution, and synthesis to completed", async () => {
  ...
  assert.ok(visibleMessages.some((message) => message.content.includes("@Worker 请开始执行：**Create report**")));
  assert.ok(visibleMessages.some((message) => message.content.includes("@Coordinator 我已完成并提交：**Create report**")));
  assert.ok(visibleMessages.some((message) => message.content.includes("已确认完成：**Create report**")));
...
test("a reviewer can own an independent task when that task does not require another review", async () => {
  let room = rooms.createRoom({ name: "Reviewer task", creator: { sessionId: "review-task-lead", role: "coordinator" } });
  room = rooms.addRoomMember(room.id, { sessionId: "review-task-agent", role: "reviewer" });
  room = rooms.configureRoomCoordination(room.id, { mode: "team", coordinatorSessionId: "review-task-lead" });
  const reviewer = room.members.find((member) => member.binding.sessionId === "review-task-agent");
  assert.ok(reviewer);
  const store = new stores.TeamRunStore({ roomsRoot: root });
  const router = new FakeRouter();
  const service = new coordinatorModule.TeamCoordinatorService({ store, router, uuid: testUuid });
  globalThis.__pioraTeamCoordinator = service;
  globalThis.__pioraTeamRunStore = store;
  const state = await service.createRun({ roomId: room.id, objective: "Independent review", coordinatorMemberId: room.coordination.coordinatorMemberId, createdBy: { kind: "user", id: "user" } });
  const planning = router.commands.at(-1);
  await service.onSessionCommandEvent({ cursor: 1, type: "prompt_started", sessionId: "review-task-lead", commandId: planning.commandId, runId: testUuid(), timestamp: Date.now() });
  const prompt = promptRuns.beginPromptRun("review-task-lead");
  promptContexts.bindTeamPromptContext(prompt, planning.input.teamExecution);
  const planned = await tools.submitTeamPlan("review-task-lead", "plan", {
    objective: state.objective,
    assumptions: [],
    successCriteria: [{ id: "reviewed", description: "Review completed" }],
    tasks: [{
      id: "independent-review",
      title: "Independent review",
      description: "Inspect the delivery independently",
      acceptanceCriteria: ["Review is recorded"],
      requiredCapabilities: ["verification"],
      dependsOn: [],
      preferredMemberId: reviewer.memberId,
      reviewRequired: false,
    }],
  });
  await promptRuns.finishPromptRun(prompt, "idle");
  const task = Object.values(planned.tasks)[0];
  assert.equal(task.preferredMemberId, reviewer.memberId);
  assert.equal(task.reviewPolicy.required, false);
  assert.deepEqual(task.reviewPolicy.reviewerMemberIds, []);
});
...
test("review changes_requested requeues only the reviewed task and increments attempt and round", async () => {
  ...
  assert.equal(router.commands.some((command) => command.input.targetSessionId === "review-reviewer"), false);
  ...
  let visibleMessages = rooms.listRoomMessages(room.id);
  assert.ok(visibleMessages.some((message) => message.author.id === "review-lead" && /^@.+ 请开始执行/u.test(message.content)));
  assert.ok(visibleMessages.some((message) => message.author.id === "review-worker" && /^@.+ 我已完成并提交/u.test(message.content)));
  assert.ok(visibleMessages.some((message) => message.author.id === "review-lead" && /^@.+ 请现在审查 [^@]+ 已提交的/u.test(message.content)));
  ...
  state = store.getTeamRun(room.id, state.id);
  const task = Object.values(state.tasks)[0];
  visibleMessages = rooms.listRoomMessages(room.id);
  assert.ok(visibleMessages.some((message) => message.author.id === "review-reviewer" && /^@.+ 审查已完成/u.test(message.content)));
