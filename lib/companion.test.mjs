...
const interaction = loadTypeScriptModule("lib/companion-interaction.ts");
...
test("companion preferences reject invalid JSON and sanitize bounded records", () => {
  ...
  assert.equal(fallback.interactionModel, null);
  assert.equal(fallback.shareWorkContext, true);
  assert.deepEqual(fallback.library, []);
  assert.equal("care" in fallback, false);
  ...
  assert.deepEqual(normalized.todos[0], { id: "todo:restored-1", text: "ship it", completed: true, progress: 100, createdAt: 12, updatedAt: 12 });
  assert.deepEqual(normalized.phrases, [{ id: "phrase:ok", label: "Test", text: "Run tests" }]);
});

test("companion preferences v4 preserves useful settings and removes legacy care state", () => {
  ...
  assert.equal("care" in migrated, false);
  ...
  const restored = store.normalizeCompanionPreferences({
    ...
    todos: [],
    phrases: [],
    care: { fedAt: 111, wateredAt: 222, pettedAt: 333 },
    idleTricks: false,
  }, fallback);
  assert.equal(restored.version, 4);
  assert.equal("care" in restored, false);
  assert.equal(restored.idleTricks, false);

  const upgraded = store.normalizeCompanionPreferences({
    version: 4,
    open: true,
    interactionModel: { provider: "deepseek", modelId: "deepseek-v4" },
    shareWorkContext: false,
    todos: [],
    phrases: [],
    library: [{ id: "library:1", kind: "command", title: "Tests", content: "npm test", pinned: true, createdAt: 5 }],
  }, fallback);
  assert.deepEqual(upgraded.interactionModel, { provider: "deepseek", modelId: "deepseek-v4" });
  assert.equal(upgraded.shareWorkContext, false);
  assert.equal(upgraded.library[0].content, "npm test");
  assert.equal(store.normalizeCompanionPreferences({ version: 5, open: true }, fallback), fallback);
});

test("companion work rhythm and model context use real bounded task metrics", () => {
  const started = interaction.updateCompanionWorkRhythm(interaction.createCompanionWorkRhythm(1_000), {
    now: 1_000,
    active: true,
    runningTasks: [{ id: "task-1", runtime: "running", pendingApproval: false, lastPromptFailed: false }],
  });
  const finished = interaction.updateCompanionWorkRhythm(started, { now: 121_000, active: true, runningTasks: [] });
  assert.equal(finished.completedToday, 1);

  const context = interaction.buildCompanionInteractionContext({
    now: 181_000,
    rhythm: finished,
    session: {
      sessionTitle: "Release desktop companion",
      status: "running",
      stats: { sessionId: "s", userMessages: 2, assistantMessages: 2, toolCalls: 3, toolResults: 3, totalMessages: 10, tokens: { input: 100, output: 50, cacheRead: 0, cacheWrite: 0, total: 150 }, cost: 0 },
    },
    runningTasks: [{ id: "task-2", runtime: "running", pendingApproval: false, lastPromptFailed: false, startedAt: 61_000, title: "Package release", contextUsage: { percent: 20, contextWindow: 1_000, tokens: 200 } }],
    personalTasks: [{ id: "todo:1", text: "Review build", completed: false, progress: 45, createdAt: 1, updatedAt: 2 }],
    includeWorkContext: true,
  });
  assert.equal(context.currentSession.tokens, 150);
  assert.equal(context.runningTasks[0].activeMinutes, 2);
  assert.equal(context.runningTasks[0].contextTokens, 200);
  assert.deepEqual(context.personalTasks[0], { title: "Review build", progress: 45 });
});
...
test("interaction events select one-shot reaction animations", () => {
  ...
  assert.equal(companion.selectCompanionTransientSpriteState([states[0]], "poke"), states[0]);
});
