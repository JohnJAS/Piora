...
test("detects repository-specific absolute checkout paths", () => {
  const legacyCheckout = "F:" + "\\pi" + "GUI\\desktop";
  assert.deepEqual(scanText(`checkout=${legacyCheckout}`, PORTABILITY_RULES), [
    { rule: "repository-specific-absolute-path", line: 1 },
  ]);
  assert.equal(scanText("checkout=<repository-root>", PORTABILITY_RULES).length, 0);
});
