...
test("companion pet routes discover, import, list, replace, and serve an installed atlas", async (t) => {
  ...
  assert.equal(initial.installed.length, 1);
  assert.equal(initial.installed[0].id, "pekka-pal.codex-pet");
  assert.equal(initial.installed[0].sourceKind, "piora-bundled");
  assert.equal(
    initial.installed[0].atlasUrl,
  ...
  assert.equal(listed.installed.length, 2);
