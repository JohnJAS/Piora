...
function routeFixture(t) {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), "pi-gui-pet-route-"));
  ...
  const previousPiGuiHome = process.env.PI_GUI_HOME;
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.PI_GUI_HOME = runtimeHome;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousPiGuiHome === undefined) delete process.env.PI_GUI_HOME;
    else process.env.PI_GUI_HOME = previousPiGuiHome;
...
test("companion pet routes discover, import, list, replace, and serve an installed atlas", async (t) => {
  ...
  assert.equal(initial.sources[0].atlasUrl, null);
  assert.equal(initial.installed.length, 0);
  ...
  assert.equal(imported.pet.sourceKind, "pi-gui-installed");
...
test("companion pet routes do not disclose unexpected runtime errors", async () => {
  const saved = {
    PI_GUI_HOME: process.env.PI_GUI_HOME,
    USERPROFILE: process.env.USERPROFILE,
    HOME: process.env.HOME,
  };
  delete process.env.PI_GUI_HOME;
