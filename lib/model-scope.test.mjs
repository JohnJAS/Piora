...
test("a broken models.json never exposes built-in models or permits startup fallback", async () => {
  let reads = 0;
  const broken = { getError: () => "Invalid models.json schema: providers.test.models", getAvailable: async () => { reads++; return MODELS; } };
  for (const patterns of [undefined, ["anthropic/*"], ["missing/*"]]) {
    await assert.rejects(resolveVisibleModels(broken, patterns), /Invalid models.json/);
  }
