...
test("Harmony Agent exposes a bounded batch scenario plus compatible explicit phone tools", () => {
  ...
  assert.match(extension, /piora_runtime_capability name="harmony_code_check" availability="active"/);
  assert.match(extension, /systemPrompt: `\$\{event\.systemPrompt\}/);
  assert.match(extension, /selected\.some\(\(name\) => name\.startsWith\("harmony_"\)\)/);
  assert.match(extension, /selected\.includes\("piora_harmony_check"\)/);
  assert.match(extension, /Harmony phone control is available through/);
  assert.match(extension, /Piora provides native ArkTS syntax\/semantic and Code Linter checks/);
