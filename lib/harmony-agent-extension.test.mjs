...
test("acquire requires RPC UI confirmation and binds the physical lease to real run identity", () => {
  assert.match(extension, /requirePromptToolIdentity\(ctx\.sessionManager\.getSessionId\(\), toolCallId\)/);
  assert.match(extension, /ctx\.hasUI/);
  assert.match(extension, /await ctx\.ui\.confirm\(/);
...
test("screenshots can be routed to a separate perception model without entering action-model context", () => {
  ...
  assert.match(extension, /Perception model observation/);
