...
test("restores session when cwd is absent", async () => {
  ...
  assert.deepEqual(
    ...
    { entryId: null, requestedCwd: null, sessionId: "saved-session" },
  );
});
...
test("preserves a URL-encoded Windows path", async () => {
  ...
  assert.deepEqual(
    ...
    { entryId: null, requestedCwd: "C:\\Projects\\pi-web", sessionId: null },
  );
});

test("restores a searched message only together with its session", async () => {
  const { getInitialNavigation } = await loadSubject();
  assert.deepEqual(
    getInitialNavigation(new URLSearchParams({ session: "saved-session", entry: "message-entry" })),
    { entryId: "message-entry", requestedCwd: null, sessionId: "saved-session" },
  );
  assert.equal(getInitialNavigation(new URLSearchParams({ entry: "orphan" })).entryId, null);
});
