...
test("release workflow verifies and publishes updater metadata with the installer", () => {
  ...
  assert.match(releaseWorkflow, /desktop\/release\/\$\{\{ matrix\.channel \}\}\.yml/);
  assert.match(releaseWorkflow, /channel: latest/);
  assert.match(releaseWorkflow, /channel: xiaoyi-latest/);
...
test("preview workflow publishes an installable beta channel alongside the portable build", () => {
  ...
  assert.match(previewWorkflow, /desktop\/release\/\$\{\{ matrix\.channel \}\}\.yml/);
  assert.match(previewWorkflow, /channel: beta/);
  assert.match(previewWorkflow, /channel: xiaoyi-beta/);
