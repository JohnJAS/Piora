...
async function loadSubject() {
  ...
  return createJiti(import.meta.url).import("./directory-browser.ts");
}
...
test("enumerates mounted drive roots without throwing", async () => {
  const { listWindowsDrives } = await loadSubject();
  const drives = await listWindowsDrives();
  assert.ok(Array.isArray(drives));
  if (process.platform === "win32") {
    assert.ok(drives.length >= 1, "expected at least one Windows drive root");
    assert.ok(drives.some((drive) => drive.path === "C:\\"));
  }
});
