...
test("all project resource loaders and reloads enforce project trust", async () => {
  ...
  const modelRuntimeContextSource = await readFile(
    new URL("./model-runtime-context.ts", import.meta.url),
    "utf8",
  );
  ...
  assert.match(modelsSource, /createTrustedModelServices\(cwd\)/);
  assert.match(modelRuntimeContextSource, /projectTrustReloadOptions\(cwd, agentDir\)/);
  assert.match(modelRuntimeContextSource, /resourceLoaderReloadOptions: trustReloadOptions/);
