...
test("new normal sessions start with every registered session tool enabled", () => {
  ...
  assert.deepEqual(policy.enabledCapabilityIds, [
    "tool:bash",
    "tool:browser",
    "tool:harmony_observe_screen",
    "tool:piora_automation",
    "tool:piora_request_user_input",
    "tool:piora_room",
    "tool:read",
  ]);
...
test("chat-only has no ordinary or prompt-mode tools", () => {
  const catalog = capabilities.buildSessionCapabilityCatalog(tools([
    "read",
    "browser",
    "piora_goal",
...
test("legacy sessions without capability metadata preserve all registered capabilities", () => {
  const catalog = capabilities.buildSessionCapabilityCatalog(tools([
    "read",
    "browser",
    "harmony_observe_screen",
