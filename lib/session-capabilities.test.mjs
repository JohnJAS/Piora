...
test("SSH joins coding sessions while older custom tool choices remain explicit", () => {
  const catalog = capabilities.buildSessionCapabilityCatalog(tools(["read", "bash", "ssh"]), "normal");
  assert.ok(capabilities.createSessionCapabilityPolicy(undefined, catalog, "normal").enabledCapabilityIds.includes("tool:ssh"));
  const restore = (preset) => capabilities.restoreSessionCapabilityPolicy([{
    type: "custom", customType: capabilities.SESSION_CAPABILITY_ENTRY_TYPE,
    data: { version: 1, revision: 2, preset, enabledCapabilityIds: ["tool:read", "tool:bash"], knownCapabilityIds: ["tool:read", "tool:bash"], updatedAt: "2026-09-17T00:00:00.000Z" },
  }], catalog);
  assert.ok(restore("coding").enabledCapabilityIds.includes("tool:ssh"));
  assert.equal(restore("custom").enabledCapabilityIds.includes("tool:ssh"), false);
});
