...
test("a clean open document follows newer disk content", () => {
  const draft = createMarkdownDraft(item, async (value) => ({ ...item, ...value, updatedAt: 3 }), memory());
  draft.syncMetadata({ ...item, title: "External", content: "changed elsewhere", updatedAt: 2 });
  assert.equal(draft.getSnapshot().title, "External");
  assert.equal(draft.getSnapshot().content, "changed elsewhere");
  assert.equal(draft.getSnapshot().dirty, false);
  draft.dispose();
});
test("recovery storage failures are visible while the in-memory draft remains intact", () => {
  const broken = { getItem: () => null, setItem: () => { throw new Error("quota"); }, removeItem: () => {} };
  const draft = createMarkdownDraft(item, async (value) => ({ ...item, ...value, updatedAt: 2 }), broken);
  draft.update({ content: "must survive" });
  assert.equal(draft.getSnapshot().content, "must survive");
  assert.match(draft.getSnapshot().error, /恢复副本/);
  draft.dispose();
});
