...
test("the real extension suppresses repeat failures across context events and permits a newly attached retry", async () => {
  ...
  const data = (await sharp({ create: { width: 2, height: 2, channels: 3, background: "white" } }).png().toBuffer()).toString("base64");
  const imageMessage = { role: "user", content: [{ type: "text", text: "inspect" }, { type: "image", mimeType: "image/png", data }], timestamp: 1 };
