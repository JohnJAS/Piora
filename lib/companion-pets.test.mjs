...
function fixture(t) {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), "pi-gui-pets-"));
  ...
  return {
    ...
    runtimeHome,
    codexHome,
    environment: { PI_GUI_HOME: runtimeHome, CODEX_HOME: codexHome },
  };
}
...
test("discovers valid Codex pets and reports invalid packages without exposing them", (t) => {
  ...
  assert.equal(result.sources[0].atlasUrl, null);
...
test("imports a current Codex manifest with a nested asset, custom frame grid, and animations", (t) => {
  ...
  const installedFolder = path.join(getPiGuiPetsDirectory(environment), "current-pet");
...
test("imports a Codex pet ZIP from a wrapper folder and validates its contents", async (t) => {
  ...
  assert.ok(fs.existsSync(path.join(getPiGuiPetsDirectory(environment), "zip-fox", "pet.json")));
  assert.ok(fs.existsSync(path.join(getPiGuiPetsDirectory(environment), "zip-fox", "spritesheet.webp")));
});
...
test("discovers and imports local Codex built-in cache pets and legacy avatars", (t) => {
  ...
  assert.deepEqual(
    fs.readFileSync(path.join(getPiGuiPetsDirectory(environment), "dewey", "spritesheet.webp")),
    builtinBytes,
  );
});
...
test("atomically imports and replaces a Codex pet using Pi GUI-owned storage", (t) => {
  ...
  assert.equal(first.pet.source, "pi-gui");
  assert.equal(first.pet.installed, true);
  assert.equal(first.pet.atlasUrl, "/api/companion-pets/focus-fox/spritesheet");

  const installFolder = path.join(getPiGuiPetsDirectory(environment), "focus-fox");
  ...
  assert.equal(
    fs.readdirSync(getPiGuiPetsDirectory(environment)).some((name) => name.startsWith(".import-") || name.startsWith(".backup-")),
    false,
  );
});
...
test("does not replace a destination that changes after the initial safety check", (t) => {
  ...
  const destination = path.join(getPiGuiPetsDirectory(environment), "swap-fox");
...
test("serves only installed pets and revalidates bytes before returning", (t) => {
  ...
  fs.writeFileSync(
    path.join(getPiGuiPetsDirectory(environment), "safe-pet", "spritesheet.png"),
...
test("uses CODEX_HOME and runtime-home conventions", (t) => {
  ...
  assert.equal(
    ...
    path.join(runtimeHome, ".pi", "agent", "pi-gui", "pets"),
  );
});
