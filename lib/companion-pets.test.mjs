...
test("resolves bundled pets from the packaged runtime root", () => {
  const packagedRuntimeRoot = path.resolve("Z:/synthetic-piora/runtime.asar");
  assert.equal(
    getBundledPioraPetsDirectory({ PIORA_WEB_RUNTIME_ROOT: packagedRuntimeRoot }),
    path.join(packagedRuntimeRoot, "public", "companion-pets", "bundled"),
  );
});
...
test("atomically imports and replaces a Codex pet using Pi GUI-owned storage", (t) => {
  ...
  const listing = listCompanionPets(environment);
  assert.equal(listing.installed.length, 10);
  assert.equal(listing.installed.filter((pet) => pet.sourceKind === "piora-bundled").length, 9);
