  ...
  try {
    ...
    await page.route("https://models-settings.test/**", async route => {
      ...
      if (url.pathname === "/api/models-config") {
        if (method === "GET") { if (corrupt) return route.fulfill({ status: 422, json: { error: "无法读取 models.json，请修复后重新加载" } }); data = config; }
        else {
    ...
    // Blank rows have no server ID: both deletion surfaces must work locally,
    // including multiple blank rows without accidentally deleting siblings.
    await page.getByText(/^\+\s*模型$/).first().click();
    await page.locator('input[placeholder="model-id"]').waitFor();
    assert.equal(await page.locator('input[placeholder="model-id"]').inputValue(), "");
    assert.equal(await page.locator('input[placeholder="model-id"]').getAttribute("aria-invalid"), "true");
    await page.getByRole("button", { name: "保存", exact: true }).click();
    await page.getByRole("alert").filter({ hasText: "模型 ID 不能为空，请填写后再保存。" }).waitFor();
    assert.equal(mutations.filter(({ method }) => method === "PUT").length, 0, "blank model IDs must be explained before any save request");
    await page.getByRole("button", { name: "移除", exact: true }).click();
    await page.locator('input[placeholder="model-id"]').waitFor({ state: "hidden" });
    await page.getByText(/^\+\s*模型$/).first().click();
    await page.getByText(/^\+\s*模型$/).first().click();
    await modelRow("新模型").getByRole("button", { name: "删除", exact: true }).click();
    assert.equal(await page.getByText("新模型", { exact: true }).count(), 1);
    await modelRow("新模型").getByRole("button", { name: "删除", exact: true }).click();
    await page.getByText("新模型", { exact: true }).waitFor({ state: "hidden" });
    assert.equal(mutations.length, 0, "draft deletion must not send an empty ID or delete the provider");
    ...
    corrupt = true;
    await page.reload();
    await page.getByRole("alert").filter({ hasText: "无法读取 models.json" }).waitFor();
    assert.equal(await page.getByRole("button", { name: "保存", exact: true }).isDisabled(), true);
    assert.equal(await modelRow("备用渠道").isVisible(), false);
    corrupt = false;
