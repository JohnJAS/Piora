...
export async function ensureDefaultShell(cwd: string, native = false): Promise<ManagedShellSession> {
  const validated = await validateTerminalCwd(cwd);
  const key = `${native ? "native:" : "managed:"}${process.platform === "win32" ? validated.toLowerCase() : validated}`;
  const defaults = globalThis.__pioraDefaultShells ??= new Map();
  const existing = defaults.get(key); if (existing) return existing;
  const pending = (async () => { const sessions = await listShells(validated, native); return sessions.length ? getShell(sessions[0].id) : createShell(validated, null, validated, native); })().finally(() => defaults.delete(key));
...
export async function listShells(cwd: string, native?: boolean): Promise<ShellSession[]> {
  const validated = await validateTerminalCwd(cwd);
  ...
  return sessions.filter(session => !session.closed && (native === undefined || Boolean(session.profile.native) === native) && key(session.workspaceCwd || session.initialCwd) === key(validated)).map(session => registry().get(session.id)?.snapshot().session || session);
}
