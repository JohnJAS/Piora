...
test("tab and conversation switches attach only the selected manual browser page", async (t) => {
  const { action, viewport, views, win, state } = setup(t);
  await action({ action: "set_session", sessionId: "conversation-one" });
  await action({ action: "navigate", url: "https://login.example.test/" });
  const firstTab = state().activeTabId;
  const firstPage = views.find((view) => view.webContents.url === "https://login.example.test/");
  await action({ action: "new_tab", url: "https://login.example.test/second" });
  const secondPage = views.find((view) => view.webContents.url === "https://login.example.test/second");
  viewport(true);
  assert.deepEqual(win.contentView.children, [secondPage]);
  await action({ action: "switch_tab", tabId: firstTab });
  assert.deepEqual(win.contentView.children, [firstPage]);
  viewport(false);
  await action({ action: "set_session", sessionId: "conversation-two" });
  await action({ action: "navigate", url: "https://manual.example.test/" });
  assert.equal(win.contentView.children.length, 0);
  viewport(true);
