...
test("file mentions escape clipped panes and template optimization uses the displayed model", { timeout: 120000 }, async () => {
  ...
  try {
    ...
    await writeFile(path.join(root, "entry.tsx"), `import React from "react";import {createRoot} from "react-dom/client";import {I18nProvider} from ${JSON.stringify(path.join(repo, "hooks/useI18n.tsx"))};import {ChatInput} from ${JSON.stringify(path.join(repo, "components/ChatInput.tsx"))};import {SystemPromptEditor} from ${JSON.stringify(path.join(repo, "components/SystemPromptEditor.tsx"))};function App(){const [revision,setRevision]=React.useState(0);window.remountComposer=()=>setRevision(n=>n+1);return location.pathname==="/editor"?<SystemPromptEditor effectivePrompt={null}/>:<div id="clipped" style={{position:"absolute",bottom:20,left:24,width:"min(650px, calc(100vw - 48px))",height:150,overflow:"hidden",transform:"translateZ(0)"}}><ChatInput key={revision} variant="launcher" cwd="C:/Workspace/sample" isStreaming={false} onSend={()=>{window.sends=(window.sends||0)+1}} onAbort={()=>{}}/></div>};createRoot(document.getElementById("root")).render(<I18nProvider><App/></I18nProvider>);`);
    ...
    const indexRequests = [];
    const firstIndexRequested = Promise.withResolvers();
    const releaseIndex = Promise.withResolvers();
    ...
    await page.route("http://composer-picker.test/**", async (route) => {
      ...
      if (url.pathname === "/api/file-index") {
        indexRequests.push(url.href);
        firstIndexRequested.resolve();
        if (indexRequests.length === 1) await releaseIndex.promise;
    ...
    await firstIndexRequested.promise;
    assert.equal(await menu.count(), 0, "index preloads before the user opens @");
    await input.fill("看看 @");
    await menu.waitFor();
    assert.equal(indexRequests.length, 1, "opening while preloading shares the same request");
    ...
    await page.evaluate(() => window.remountComposer());
    await input.fill("@file");
    await menu.getByRole("option").first().waitFor();
    assert.equal(indexRequests.length, 1, "another session in the project reuses the loaded index");
