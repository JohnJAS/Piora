...
const tools = [
  ...
  { name: "piora_harmony_check", active: true },
...
test("packaged tool verification follows compact gateway registration and optional-extension defaults", () => {
  assert.equal(verifyPackagedCoreTools(tools).filter((entry) => entry.active).length, 3);
  assert.throws(() => verifyPackagedCoreTools(tools.filter((entry) => entry.name !== "harmony_control")), /failed to load/);
  assert.throws(() => verifyPackagedCoreTools(tools.filter((entry) => entry.name !== "piora_harmony_check")), /failed to load/);
