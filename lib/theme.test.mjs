...
test("keeps appearance discoverable from the desktop feature menu and settings, but not the top bar", () => {
  ...
  assert.ok((source.match(/openAppearanceSettings\(\)/g) ?? []).length >= 2);
  assert.match(desktopSource, /sendMenuAction\("appearance"\)/);
  assert.match(settingsSource, /key:\s*"appearance"[\s\S]*onOpen:\s*onOpenAppearance/);
  assert.match(source, /onOpenAppearance=\{\(\) => \{ setSettingsDialogOpen\(false\); openAppearanceSettings\(\); \}\}/);
  assert.doesNotMatch(source, /sidebar-user-menu/);
  assert.doesNotMatch(source, /themeBtnRef|app-topbar-appearance|openAppearanceSettings\(event\.currentTarget\)/);
  assert.match(settingsSource, /role="region"/);
  assert.doesNotMatch(settingsSource, /aria-modal|createPortal/);
  assert.match(source, /role="dialog"/);
  assert.match(source, /data-theme-id=\{preset\.id\}/);
});

test("offers four one-click visual looks backed by local artwork", () => {
