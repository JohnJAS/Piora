...
test("keeps appearance discoverable from the sidebar menu, settings, and top bar", () => {
  const source = readFileSync(resolve(projectRoot, "components/AppShell.tsx"), "utf8");
  assert.ok((source.match(/openAppearanceSettings\(event\.currentTarget\)/g) ?? []).length >= 1);
  assert.ok((source.match(/openAppearanceSettings\(\)/g) ?? []).length >= 2);
  assert.match(source, /sidebar-user-menu/);
