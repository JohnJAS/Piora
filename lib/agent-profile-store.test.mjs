...
test("a stored device session cannot be opened by a normal process", async (t) => {
  const storePath = tempStorePath(t);
  await bindSessionAgentRuntimeProfile("device-session", "device-control", storePath);
  await assert.rejects(
    resolveSessionAgentRuntimeProfile("device-session", "normal", storePath),
    (error) => error?.code === "SESSION_PROFILE_MISMATCH",
  );
});

test("session-list visibility treats only unbound legacy sessions as normal", () => {
  ...
  assert.equal(isSessionVisibleInAgentRuntimeProfile("device", "normal", store), false);
