...
test("optionally includes unstaged files and pushes through the configured upstream", async () => {
  const root = createRepository();
  const remote = fs.mkdtempSync(path.join(os.tmpdir(), "piora-git-remote-"));
  try {
    git(remote, "init", "--bare");
    git(root, "remote", "add", "origin", remote);
    const branch = git(root, "branch", "--show-current");
    git(root, "push", "--set-upstream", "origin", branch);
    fs.writeFileSync(path.join(root, "included.txt"), "included\n");
    await commitGit(root, "include worktree", false, true);
    assert.equal(git(root, "show", "--pretty=", "--name-only", "HEAD"), "included.txt");
    await pushGit(root);
    assert.equal(git(remote, "log", "-1", "--pretty=%B"), "include worktree");
  } finally {
...
test("allows staging resolved conflicts while blocking commits with unresolved paths", async () => {
  const root = createRepository();
  try {
    ...
    await assert.rejects(commitGit(root, "unresolved"), (error) => error?.code === "conflict");
    fs.writeFileSync(path.join(root, "tracked.txt"), "resolved\n");
    await stageGitPaths(root, ["tracked.txt"]);
    assert.equal(git(root, "diff", "--name-only", "--diff-filter=U"), "");
    assert.match(await commitGit(root, "resolved"), /^[a-f0-9]{40}$/);
  } finally { fs.rmSync(root, { recursive: true, force: true }); }
});
