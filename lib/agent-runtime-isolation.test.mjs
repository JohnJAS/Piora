...
test("device-control admits only its first-party extension allow-list", () => {
  assert.deepEqual(
    ...
    ["piora-goal.ts", "piora-harmony.ts", "piora-plan.ts"],
  );
  assert.deepEqual(
...
test("ordinary sessions expose every first-party extension to the load plan", () => {
  assert.deepEqual(
    ...
    ["piora-automations.ts", "piora-browser.ts", "piora-file-changes.ts", "piora-goal.ts", "piora-harmony.ts", "piora-plan.ts", "piora-room.ts", "piora-user-input.ts", "piora-vision-agent.ts"],
  );
});
