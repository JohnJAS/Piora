...
test("full-context diffs reveal omitted unchanged lines and report the file length", async (t) => {
  ...
  const fixtureDiagnostics = JSON.stringify({
    compact,
    full,
    root,
    ...
    status: runGit("status", "--porcelain=v1").toString("utf8"),
    diff: runGit("diff", "--no-color", "--no-ext-diff", "--unified=3", "--", "long.txt").toString("utf8"),
  });
  assert.equal(compact.supported, true, fixtureDiagnostics);
