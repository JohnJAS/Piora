...
test("production chips/settings: guarded edits, keyboard, durable failed sends, refresh and source isolation",{timeout:180_000},async()=>{
  ...
  try{
    ...
    await context.route("http://localhost:31997/**",async route=>{
      ...
      if(url.pathname.endsWith("/reply-suggestions")||url.pathname.endsWith("/preview")){requests.push({path:url.pathname,body:route.request().postDataJSON()});if(url.pathname.endsWith("/reply-suggestions")&&failures.length){const status=failures.shift();return route.fulfill({status,json:{code:status===409?"stale_source":"timeout"}})}if(url.pathname.includes("/two/"))await new Promise(resolve=>{delayed=resolve});return route.fulfill({json:result}).catch(()=>{});}
    ...
    await page.getByRole("status").filter({hasText:"快捷回复设置已保存，将用于后续提取。"}).waitFor();
    // Editing clears the previous acknowledgement. A denied write must never
    // display success or replace the previously saved prompt.
    const promptEditor=page.getByRole("textbox",{name:"提取提示词",exact:true});
    await promptEditor.fill("修改后的提取规则");
    assert.equal(await page.getByText("已保存",{exact:true}).count(),0);
    await page.evaluate(()=>{window.originalStorageSet=Storage.prototype.setItem;Storage.prototype.setItem=function(key,value){if(key==='piora-reply-suggestions-settings-v1')throw new DOMException('Denied','QuotaExceededError');return window.originalStorageSet.call(this,key,value)}});
    await page.getByRole("button",{name:"保存设置"}).click();
    await page.getByRole("alert").filter({hasText:"保存失败"}).waitFor();
    assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('piora-reply-suggestions-settings-v1')).systemPrompt),"只提取明确选项，保留用户原意。");
    await page.evaluate(()=>{Storage.prototype.setItem=window.originalStorageSet});
    await page.getByRole("button",{name:"保存设置"}).click();
    await page.getByRole("status").filter({hasText:"快捷回复设置已保存"}).waitFor();
    assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('piora-reply-suggestions-settings-v1')).systemPrompt),"修改后的提取规则");
    await page.reload();
    assert.equal(await page.getByRole("textbox",{name:"提取提示词",exact:true}).inputValue(),"修改后的提取规则");
    ...
    assert.equal(requests[0].body.model.modelId,"extract");assert.equal(requests[0].body.systemPrompt,"修改后的提取规则");
    failures.push(409,409,504);
    await page.goto("http://localhost:31997/?dark");
    await page.waitForRequest(r=>r.url().includes("/one/reply-suggestions"));
    assert.equal(await page.getByText("正在提取",{exact:false}).count(),0);
    await page.getByRole("status").filter({hasText:"60 秒内未返回结果"}).waitFor();
    assert.equal(requests.filter(r=>r.path.includes('/one/reply-suggestions')).length,3,"settlement conflicts retry but provider failures remain visible");
    ...
    // Re-enabling the source can briefly expose the previous result before its
    // cache refresh. Capture visible content and geometry in one settled frame.
    const visibleChips=await page.waitForFunction(()=>{
      const section=document.querySelector('section[aria-label="快捷回复"]');
      const nodes=Array.from(section?.querySelectorAll('button')??[]);
      if(nodes.length!==4||nodes.some(node=>!node.checkVisibility()))return false;
      return {text:section.innerText,bounds:nodes.map(node=>({top:node.getBoundingClientRect().top,radius:getComputedStyle(node).borderRadius}))};
    });
    const {text,bounds}=await visibleChips.jsonValue();await visibleChips.dispose();
    assert.equal(text,"浅色主题\n深色主题\n添加搜索\n支持导出");
    ...
    await page.setViewportSize({width:1100,height:880});await page.goto("http://localhost:31997/?settings");await page.getByRole("textbox",{name:"提取提示词",exact:true}).waitFor();
    await page.getByRole("textbox",{name:"提取提示词",exact:true}).fill("测试结束后的提取规则");
    await page.getByRole("button",{name:"保存设置"}).click();
    await page.getByRole("status").filter({hasText:"快捷回复设置已保存"}).waitFor();
    await page.screenshot({path:path.join(output,"settings-light.png"),fullPage:true});
    const beforeRetry=requests.length;
    failures.push(409,409,409,409);
    await page.goto("http://localhost:31997/?dark");
    await page.getByRole("status").filter({hasText:"会话尚未就绪"}).waitFor();
    assert.equal(requests.length-beforeRetry,4,"settlement retries are bounded and exhaustion exposes a retry control");
