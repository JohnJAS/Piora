...
test("desktop persists the selected Pi directory and offers compatible legacy migration", () => {
  assert.match(main, /choosePiAgentDirectory/);
  assert.match(main, /directoryHasData\(legacyDirectory\)/);
  assert.match(main, /migratePiDataDirectory/);
  assert.match(state, /piAgentDirectory\?: string/);
