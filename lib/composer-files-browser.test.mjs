...
test("composer pastes real local files, preserves binary uploads and recovers file references", { timeout: 120000 }, async () => {
  ...
  try {
    ...
    await page.route("http://composer-files.test/**", async route => {
      ...
      if (url.pathname === "/api/prompt-files") {
        ...
        if (failUpload) return route.fulfill({ status: 500, json: { error: "test storage unavailable" } });
        return route.fulfill({ json: { files: await savePromptFiles(body.getAll("files"), path.join(root, "stored")) } });
      }
    ...
    await page.goto("http://composer-files.test/");
    ...
    await send.click();
    await page.waitForFunction(() => window.sends.length === 1 && document.querySelector("textarea").value === "请分析附件");
