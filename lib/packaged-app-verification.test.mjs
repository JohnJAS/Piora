...
test("packaged dependency audit finds scoped and nested unused peer packages", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "pi-gui-package-audit-"));
...
test("packaged background manifest and all 20 assets match source byte-for-byte", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "pi-gui-background-package-"));
  t.after(() => rm(root, { recursive: true, force: true }));
...
test("packaged extension fixture is an external Pi package, not a piGUI SubAgent", async () => {
  ...
  assert.equal(manifest.name, "@pi-gui/packaged-extension-verification-fixture");
