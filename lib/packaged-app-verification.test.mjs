...
test("standalone staging treats public assets as required release input", async () => {
  ...
  assert.match(stagingScript, /const devecoCliRuntimeRoot = join\(projectRoot, "node_modules", "@deveco", "deveco-cli"\)/);
  assert.match(stagingScript, /collectRuntimeDependencyAssets\(\[[\s\S]*piAiProviderRuntimeRoot,[\s\S]*hypiumRuntimeRoot,[\s\S]*devecoCliRuntimeRoot,[\s\S]*\]\)/);
...
test("packaged runtime verifies every first-party extension and workflow tool state", async () => {
  ...
  assert.match(verifier, /node_modules\/@deveco\/deveco-cli\/package\.json/);
  ...
  assert.match(verifier, /import \{ verifyBrandStartupAssets \} from "\.\/verify-brand-startup-assets\.mjs"/);
  assert.match(verifier, /await verifyBrandStartupAssets\(projectRoot, resourcesRoot\)/);
  ...
  assert.match(
    await readFile(new URL("../lib/harmony/check-runtime.ts", import.meta.url), "utf8"),
    /process\.env\.PIORA_WEB_RUNTIME_ROOT\?\.trim\(\) \|\| process\.cwd\(\)/,
  );
});
