...
test("standalone staging collects opaque provider dependency closures", async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      "Pi AI provider dependency direct-runtime",
      "Pi AI provider dependency nested-runtime",
      "Pi AI provider dependency peer-runtime",
      "Pi AI provider dependency provider-runtime",
    ],
  );
...
test("packaged background manifest and all 30 assets match source byte-for-byte", async (t) => {
  ...
  assert.equal(verified.backgroundCount, 30);
...
test("standalone staging treats public assets as required release input", async () => {
  ...
  assert.match(stagingScript, /collectRuntimeDependencyAssets\(\[piAiProviderRuntimeRoot\]\)/);
  assert.match(
    stagingScript,
