...
test("maps the shared TaskRun waiting phase to needs-input attention", () => {
  const taskRun = {
    schemaVersion: 1,
    taskId: "goal-1",
    source: "goal",
    sessionId: "session-a",
    objective: "Publish with chosen visibility",
    phase: "waiting_user",
    attempt: 1,
    createdAt: 1,
    updatedAt: 2,
    reason: "Choose public or private",
    evidence: [],
    artifacts: [],
  };
  assert.equal(deriveTaskStatus(input({ taskRun })).attention, "needs_input");
  assert.equal(deriveTaskStatus(input({ taskRun, isViewing: true })).attention, "none");
});
