...
test("goal tool records checkpoints and evidence before completing", async () => {
  ...
  promptRuns.beginPromptRun("session-tool");
  await harness.tool.execute("tool-start", {
    action: "start",
    objective: "Ship a verified optional-workflow MVP",
    successCriteria: ["The implementation passes its checks"],
  ...
  assert.deepEqual(goals.getGoalRun("session-tool")?.successCriteria, ["The implementation passes its checks"]);
