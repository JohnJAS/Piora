...
test("Harmony capability is appended to custom system prompts only while its tool is active", async () => {
  ...
  assert.equal(tools[1].name, "piora_harmony_check");
  ...
  const checking = await handler(event("CUSTOM SYSTEM PROMPT", ["piora_harmony_check"]), {});
  assert.match(checking.systemPrompt, /piora_runtime_capability name="harmony_code_check" availability="active"/);
  assert.match(checking.systemPrompt, /piora_harmony_check/);
  assert.doesNotMatch(checking.systemPrompt, /harmony_phone_operator/);
  assert.equal(await handler(event(checking.systemPrompt, ["piora_harmony_check"]), {}), undefined);
});
