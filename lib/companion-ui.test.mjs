...
const REQUIRED_MESSAGES = [
  ...
  "companion.source.piGuiInstalled",
...
test("companion remains a local UI surface wired to the existing chat input", async () => {
  const [component, input, shell, chat, preferences, petLoader, settings, companionSettings, desktop] = await Promise.all([
  ...
  assert.match(settings, /onOpenCompanion/);
  ...
  assert.match(desktop, /sendMenuAction\("companion-settings"\)/);
