...
test("companion uses independent pet, bubble, and panel surfaces", async () => {
  const [desktop, preload, pet, petStyles, bubble, panel, panelStyles, layout, backgrounds] = await Promise.all([
  ...
  assert.match(bubble, /new EventSource\("\/api\/companion\/events"\)/);
