...
test("companion remains a local UI surface wired to the existing chat input", async () => {
  const [component, input, shell, chat] = await Promise.all([
  ...
  assert.match(component, /COMPANION_STORAGE_KEY/);
  ...
  assert.match(component, /sourceKind: pet\.sourceKind/);
  assert.match(component, /pet\.sourceKey/);
