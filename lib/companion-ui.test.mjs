...
test("companion uses independent pet, bubble, and panel surfaces", async () => {
  ...
  assert.match(desktop, /window\.on\("close"[\s\S]*?companionPanelKeepVisibleUntilClose = false[\s\S]*?window\.hide\(\)[\s\S]*?window\.unmaximize\(\)/);
  ...
  assert.match(togglePanelSource, /isVisible\(\) \|\| companionPanelWindow\.isMinimized\(\) \|\| companionPanelWindow\.isMaximized\(\)/);
  assert.ok(togglePanelSource.indexOf("companionPanelWindow.hide()") < togglePanelSource.indexOf("companionPanelWindow.unmaximize()"));
  ...
  assert.match(panel, /const updatePersonalityDraft/);
  ...
  assert.doesNotMatch(jsonWorkbench, /renameDraft|renameHint|renamePrompt|onDoubleClick|window\.prompt/);
