...
test("companion uses independent pet, bubble, and panel surfaces", async () => {
  const [desktop, preload, pet, petStyles, bubble, bubblePage, panel, panelStyles, layout, backgrounds] = await Promise.all([
  ...
  assert.match(desktop, /window\.on\("blur"[\s\S]*?window\.hide\(\)/);
  ...
  assert.match(petStyles, /\.pet\s*\{[\s\S]*?width:\s*92px;[\s\S]*?clip-path:\s*ellipse/);
...
test("companion runtime is persisted, bounded, and model-driven", async () => {
  ...
  assert.match(runtime, /getRuntimeAgentDataDirectory/);
