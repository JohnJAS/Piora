...
const REQUIRED_MESSAGES = [
  ...
  "companion.care.feed",
  "companion.care.water",
  "companion.care.pet",
  "companion.care.hungerHint",
  "companion.care.thirstHint",
  "companion.care.affectionHint",
];
...
test("companion remains a local UI surface wired to the existing chat input", async () => {
  const [component, input, shell, chat, preferences, petLoader, settings, companionSettings, desktop, desktopPet, desktopPetStyles] = await Promise.all([
  ...
  assert.match(desktopPet, /data-kind=\{item\.id === "companion-status" \? "status" : "task"\}/);
  ...
  assert.match(desktopPet, /data-testid="companion-care-bar"/);
  assert.match(desktopPet, /pickCompanionSpeechLine/);
  assert.match(desktopPet, /getCompanionCareLevels/);
  assert.match(desktopPet, /onClick=\{\(\) => react\("poke", "poke"\)\}/);
  ...
  assert.match(desktopPetStyles, /\.activityBubbles\s*\{[^}]*bottom:\s*178px/s);
  ...
  assert.match(desktopPetStyles, /\.careBar/);
  assert.match(desktopPetStyles, /\.careButton/);
