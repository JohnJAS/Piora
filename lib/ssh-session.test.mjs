...
const { Server } = ssh2;
const { SSHSession, listSSHSessions, createSSHSession, closeSSHSession } = await jiti.import("./ssh/session-manager.ts");
...
async function fixture(t) {
  ...
  return { session, options, clients, counts: () => ({ shells, execs }) };
}
...
test("SSH binding and unbinding invalidate idle task runtimes and refuse busy or duplicate associations", { timeout: 120000 }, async t => {
  ...
  await changeSSHBinding(session, id);
  assert.equal(session.snapshot().agentSessionId, id);
  assert.equal(destroyed, 1);
  registry.set(id, { ...idle, getRuntime: () => "running" });
  await assert.rejects(changeSSHBinding(session), error => error.code === "busy");
  assert.equal(session.snapshot().agentSessionId, id);
  registry.set(id, idle);
  await changeSSHBinding(session);
  assert.equal(session.snapshot().mode, "independent");
  assert.equal(destroyed, 2, "unlink must rebuild the local Bash runtime too");
  const other = createSSHSession(options); other.bindAgent(id);
  t.after(() => closeSSHSession(other.id));
  await assert.rejects(changeSSHBinding(session, id), error => error.code === "alreadyBound");
  other.unbindAgent();
  (globalThis.__piStartLocks ??= new Map()).set(id, Promise.resolve({}));
  await assert.rejects(changeSSHBinding(session, id), error => error.code === "busy");
  assert.equal(session.snapshot().mode, "independent");
});
