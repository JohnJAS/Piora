...
test("rejects invalid, oversized, and too many image attachments", async () => {
  const {
    ...
    MAX_ATTACHED_IMAGES,
    validateAgentImages,
    validateAttachedImageTotalBytes,
  } = await loadSubject();

  assert.equal(MAX_ATTACHED_IMAGE_BYTES, 100 * 1024 * 1024);
  assert.equal(validateAgentImages([image]), null);
  assert.match(validateAgentImages([{ ...image, mimeType: "text/plain" }]), /valid base64 image/);
  assert.match(validateAgentImages([{ ...image, data: "invalid" }]), /100MB/);
  assert.match(validateAttachedImageTotalBytes([MAX_ATTACHED_IMAGE_TOTAL_BYTES, 1]), /100MB/);
