...
test("one Session drains next_turn messages FIFO while two Sessions overlap", async () => {
  ...
  try {
    ...
    const receipts = await Promise.all([
      ...
      router.dispatchSessionMessage({ targetSessionId: "session-a", content: "two", source: "room", idempotencyKey: "a-2" }),
...
test("cancelCommand removes only the addressed queued command", async () => {
  ...
  try {
    ...
    const router = makeRouter(root, new Map([[session.sessionId, session]]));
    const target = await router.dispatchSessionMessage({ targetSessionId: session.sessionId, content: "cancel this", source: "room", idempotencyKey: "cancel-target" });
