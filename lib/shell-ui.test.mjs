...
test("legacy components preserve submissions and terminal settings explain native history", { timeout: 180000 }, async () => {
  ...
  try {
    ...
    await page.getByRole("heading", { name: "终端", exact: true }).first().waitFor();
    await page.getByText("自动开启 PSReadLine 历史提示", { exact: false }).waitFor();
    assert.equal(await page.getByRole("combobox").count(), 0, "native terminal settings have no AI model or shell selector");
    assert.equal(await page.getByRole("switch").count(), 0, "history is managed by PowerShell itself");
