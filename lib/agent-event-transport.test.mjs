...
test("metrics survive coalesced deltas, reconnects and silence without crossing generations", () => {
  const { transport, decoded } = setup();
  const message = { role: "assistant", content: [{ type: "text", text: "before switch" }], streamingMetrics: { generation: 2, tokensPerSecond: 40 } };
  transport.push({ type: "connected" });
  transport.push({ type: "message_update", message });
  assert.equal(decoded.at(-1).message.streamingMetrics.tokensPerSecond, 40);
  for (const rate of [45, 50]) {
    message.content[0].text += "x";
    message.streamingMetrics = { generation: 2, tokensPerSecond: rate };
    transport.push({ type: "message_update", message, assistantMessageEvent: { type: "text_delta", contentIndex: 0, delta: "x" } });
  }
  transport.push({ type: "message_metrics", streamingMetrics: { generation: 2, tokensPerSecond: 0 } });
  assert.equal(decoded.at(-2).message.streamingMetrics.tokensPerSecond, 50);
  assert.equal(decoded.at(-1).message.streamingMetrics.tokensPerSecond, 0);
  assert.equal(decoded.at(-1).message.content[0].text, "before switchxx");
  const count = decoded.length;
  transport.push({ type: "message_metrics", streamingMetrics: { generation: 1, tokensPerSecond: 500 } });
  assert.equal(decoded.length, count);
  transport.push({ type: "message_end", message });
  transport.push({ type: "message_metrics", streamingMetrics: { generation: 2, tokensPerSecond: 10 } });
  assert.equal(decoded.at(-1).type, "message_end");
  transport.close();
});
