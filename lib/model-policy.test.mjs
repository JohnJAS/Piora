...
test("reads a complete PI_GUI desktop default without accepting partial config", () => {
  assert.deepEqual(readPiGuiDefaultModel({
    PI_GUI_DEFAULT_PROVIDER: " deepseek ",
    PI_GUI_DEFAULT_MODEL: " deepseek-reasoner ",
  }), { provider: "deepseek", modelId: "deepseek-reasoner" });
  assert.equal(readPiGuiDefaultModel({ PI_GUI_DEFAULT_PROVIDER: "deepseek" }), undefined);
});

test("valid settings win over the PI_GUI default and DeepSeek preference", () => {
  assert.deepEqual(resolveDefaultModelPreference({
    models,
    settingsProvider: "anthropic",
    settingsModel: "claude-sonnet",
    environment: {
      PI_GUI_DEFAULT_PROVIDER: "openai",
      PI_GUI_DEFAULT_MODEL: "gpt",
    },
  }), { provider: "anthropic", modelId: "claude-sonnet" });
});

test("uses the PI_GUI default when settings are absent", () => {
  assert.deepEqual(resolveDefaultModelPreference({
    models,
    environment: {
      PI_GUI_DEFAULT_PROVIDER: "openai",
