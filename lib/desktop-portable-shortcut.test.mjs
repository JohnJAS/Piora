...
test("ZIP-extracted releases preserve the first desktop shortcut", (t) => {
  const state = fixture();
  t.after(state.cleanup);
  ...
  assert.equal(ensurePortableDesktopShortcut(unpackedOptions("0.3.2", newer)).status, "kept-existing");
  assert.equal(state.links.get(join(state.desktopDirectory, "Piora.lnk")).target, older);
});

test("later portable launches never rewrite an existing desktop shortcut", (t) => {
  const state = fixture();
  t.after(state.cleanup);
  ...
  assert.equal(ensurePortableDesktopShortcut(options(state, "0.3.2", newer)).status, "kept-existing");
  assert.equal(state.links.get(join(state.desktopDirectory, "Piora.lnk")).target, older);
  assert.equal(state.links.get(join(state.desktopDirectory, "Piora.lnk")).operation, "create");
});

test("an existing shortcut is preserved even when its target is missing", (t) => {
  const state = fixture();
  t.after(state.cleanup);
  const older = join(state.root, "Piora-0.3.1-win-x64-portable.exe");
  const newer = join(state.root, "Piora-0.3.2-win-x64-portable.exe");
  ...
  const preserved = ensurePortableDesktopShortcut(options(state, "0.3.1", older));
  assert.equal(preserved.status, "kept-existing");
  assert.equal(state.links.get(join(state.desktopDirectory, "Piora.lnk")).target, newer);
});

test("an existing unreadable desktop shortcut is left untouched", (t) => {
  const state = fixture();
  t.after(state.cleanup);
  const shortcutPath = join(state.desktopDirectory, "Piora.lnk");
  const executable = join(state.root, "Piora-0.3.2-win-x64-portable.exe");
  writeFileSync(shortcutPath, "existing shortcut");
  writeFileSync(executable, "portable");

  const result = ensurePortableDesktopShortcut(options(state, "0.3.2", executable));
  assert.equal(result.status, "kept-existing");
  assert.equal(state.links.size, 0);
  assert.equal(readFileSync(shortcutPath, "utf8"), "existing shortcut");
});
...
test("a secondary portable launch still ensures a missing shortcut before exiting", () => {
  assert.match(
    desktopMain,
