...
test("Harmony routes require the per-launch desktop token and isolated runtime profile", () => {
  assert.match(shared, /isValidDesktopToken\(request\.headers\.get\(PI_DESKTOP_TOKEN_HEADER\)\)/);
  assert.match(shared, /PIORA_RUNTIME_PROFILE !== "device-control"/);
  assert.match(shared, /publicManagerState/);
  assert.match(shared, /leases\.map\(\(\{ token, \.\.\.lease \}\)/);
  assert.match(shared, /void token;/);
  for (const source of [action, frame, events, manual, config, approval]) {
