...
test("reserves exactly 32 stable original background slots", () => {
  assert.deepEqual(subject.BACKGROUND_PRESETS.map(({ id }) => id), expectedIds);
  assert.equal(new Set(subject.BACKGROUND_PRESETS.map(({ asset }) => asset)).size, 32);
});

test("keeps all 32 completed backgrounds local-only and backed by real assets", () => {
