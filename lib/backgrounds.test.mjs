...
test("keeps the settings canvas on the shared background layer", () => {
  ...
  assert.match(
    css,
    /html\[data-app-background-active=["']true["']\]\s+\.settings-backdrop,[\s\S]*?html\[data-app-background-active=["']true["']\]\s+\.settings-embedded-section\s*\{[^}]*background:\s*transparent\s*!important/s,
  );
  assert.match(
    css,
