...
test("capability bundle export route forwards request cancellation to in-flight export work", async (t) => {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "piora-capability-route-abort-"));
  t.after(() => fs.rmSync(cwd, { recursive: true, force: true }));
  allowFileRoot(cwd);
  const controller = new AbortController();
  const responsePromise = capabilityBundleRoute.GET(new Request(
    `http://localhost:30141/api/capability-bundles?cwd=${encodeURIComponent(cwd)}`,
    { signal: controller.signal },
  ));
  queueMicrotask(() => controller.abort());
  const response = await responsePromise;
  assert.equal(response.status, 499);
  assert.match((await response.json()).error, /abort|cancel/i);
});
...
test("capability bundle export aborts while asynchronously walking local capabilities", async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "piora-capability-walk-abort-"));
  const agentDir = path.join(root, "agent");
  const homeDir = path.join(root, "home");
  const cwd = path.join(root, "project");
  const extensionDir = path.join(agentDir, "extensions", "cancel-fixture");
  fs.mkdirSync(extensionDir, { recursive: true });
  fs.mkdirSync(homeDir, { recursive: true });
  fs.mkdirSync(cwd, { recursive: true });
  fs.writeFileSync(path.join(extensionDir, "index.ts"), "export default function cancelFixture() {}\n");
  for (let index = 0; index < 50; index += 1) {
    fs.writeFileSync(path.join(extensionDir, `asset-${index}.txt`), "fixture\n".repeat(128));
  }
  ...
  const controller = new AbortController();
  const exportPromise = exportCapabilityBundle(cwd, { signal: controller.signal });
...
test("capability bundle export embeds custom project extensions and skills without environment files", async (t) => {
  ...
  const globalExtensionDir = path.join(agentDir, "extensions", "side-effect-extension");
  const skillDir = path.join(cwd, ".pi", "skills", "fixture-skill");
  const extensionLoadMarker = path.join(root, "extension-factory-ran.txt");
  fs.mkdirSync(agentDir, { recursive: true });
  fs.mkdirSync(homeDir, { recursive: true });
  fs.mkdirSync(extensionDir, { recursive: true });
  fs.mkdirSync(globalExtensionDir, { recursive: true });
  ...
  fs.writeFileSync(
    path.join(globalExtensionDir, "index.ts"),
    `import fs from "node:fs";\nfs.writeFileSync(${JSON.stringify(extensionLoadMarker)}, "loaded");\nexport default function sideEffect() {}\n`,
  );
  ...
  aborted.abort();
  await assert.rejects(
    exportCapabilityBundle(cwd, { signal: aborted.signal }),
    (error) => error instanceof Error && error.name === "AbortError",
  );

  const { bytes, manifest } = await exportCapabilityBundle(cwd);
  assert.equal(fs.existsSync(extensionLoadMarker), false, "export must not execute extension factories");
  ...
  assert.equal(imported.summary.pluginsInstalled, 2);
