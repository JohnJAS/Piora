...
const { HdcBackend, HarmonyError } = await jiti.import("./harmony/index.ts");
...
test("discovers verbose HDC targets and probes UiTest capabilities", async () => {
  ...
  const backend = backendWith(async ({ executable, args }) => {
    ...
    const values = {
      "param get const.product.model": "HUAWEI Mate 60 Pro\n",
      "param get const.product.name": "ALN-AL00\n",
  ...
  assert.equal(devices[0].model, "HUAWEI Mate 60 Pro");
...
test("uses fixed argument arrays for actions and validates identifiers", async () => {
  ...
  assert.deepEqual(calls[2], ["-t", "alpha", "shell", "uitest", "uiInput", "keyEvent", "2720"]);
  assert.deepEqual(calls[3], ["-t", "alpha", "shell", "aa", "start", "-b", "com.example.demo", "-a", "EntryAbility"]);
