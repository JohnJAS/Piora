...
function fixture({ failedLoad = false, failedWrite = false, delayedLoad = false, abortOnStop = false, failedStop = false, brand = "piora", seen = false, smoke = false } = {}) {
  ...
  for (const emitter of [window, webContents]) {
    const remove = emitter.removeListener.bind(emitter);
  ...
  window.destroy = () => {
    // Deliver close after native handles have gone away, including when
    // quitAndInstall interrupts the startup handoff.
    const closing = window.listeners("closed");
    window.removeAllListeners(); webContents.removeAllListeners();
    destroyed = true;
  ...
  const bindings = {
    ...
    loadStartupMedia: () => brand === "piora" ? ({ video: "packaged-video" }) : {}, createStartupDocument: options => { documentOptions = options; return "x".repeat(8_200_000); },
    APP_BRAND: { id: brand }, hasShownXiaoyiStartup: () => seen, markXiaoyiStartupShown: () => { marked++; }, XIAOYI_FIRST_STARTUP_MS: 5000,
    STARTUP_MEDIA_TIMEOUT_MS: 10000, STARTUP_CONTINUE_CHANNEL: "pi:startup-continue", runOptionalStartupTask, PORTABLE_SMOKE_TEST: smoke,
    writeFileSync: (path, content) => { if (failedWrite) throw new Error("read-only"); writes.push({ path, size: content.length }); },
    setTimeout: (callback, delay) => { const timer = { delay, cleared: false, run() { if (!timer.cleared) callback(); }, unref() {} }; timers.push(timer); return timer; }, clearTimeout(timer) { timer.cleared = true; },
  };
  const start = new Function(...Object.keys(bindings), compiled + ";return createStartupWindow;")(...Object.values(bindings));
  return { ...start({ warn() {}, info() {} }), webContents, loaded, writes, timers, settleLoad: () => settleLoad(), stopped: () => stopped, marked: () => marked, documentOptions: () => documentOptions };
}

test("XiaoYi first use waits five seconds from visibility despite existing Piora version history", async () => {
  const f = fixture({ brand: "xiaoyi-harness" });
  let finished = false;
  void f.finished.then(() => { finished = true; });
  assert.equal(f.timers.length, 1, "only the load watchdog runs before visibility");
  assert.equal(f.documentOptions().allowSkip, false);
  f.window.emit("ready-to-show");
  assert.equal(f.timers[0].cleared, true);
  assert.equal(f.timers[1].delay, 5000);
  f.webContents.emit("ipc-message", { senderFrame: f.webContents.mainFrame }, "pi:startup-continue");
  await new Promise(resolve => setImmediate(resolve));
  assert.equal(finished, false, "early skip or reduced-motion IPC cannot cut short static first use");
  assert.equal(f.marked(), 0);
  f.timers[1].run(); await f.finished;
  assert.equal(f.marked(), 1);
  f.ensureVisible();
  assert.equal(f.timers.length, 2, "duplicate readiness cannot restart the delay");
});

test("later XiaoYi launches and upgrades have no delay; smoke probes do not consume first use", async () => {
  for (const options of [{ seen: true }, { smoke: true }]) {
    const f = fixture({ brand: "xiaoyi-harness", ...options });
    f.window.emit("ready-to-show"); await f.finished;
    assert.equal(f.timers.length, 1);
    assert.equal(f.marked(), 0);
  }
});

test("closing or failing XiaoYi startup never consumes the first-use display", async () => {
  const closed = fixture({ brand: "xiaoyi-harness" });
  closed.window.emit("ready-to-show");
  closed.window.destroy(); await closed.finished;
  closed.timers[1].run();
  assert.equal(closed.marked(), 0);
  for (const options of [{ failedLoad: true }, { failedWrite: true }]) {
    const failed = fixture({ brand: "xiaoyi-harness", ...options });
    await failed.finished;
    failed.ensureVisible();
    assert.equal(failed.timers.length, 1);
    assert.equal(failed.marked(), 0);
  }
  const stalled = fixture({ brand: "xiaoyi-harness", delayedLoad: true, abortOnStop: true });
  stalled.timers[0].run(); await stalled.finished;
  assert.equal(stalled.marked(), 0);
});
