...
test("packaged startup verification checks each brand's exact media and rejects stale or corrupt resources", async t => {
  const root = await fixture(t);
  const packaged = resolve(root, "packaged-resources");
  for (const id of ["piora", "xiaoyi-harness"]) {
    await prepareBranding(root, id);
    ...
    await verifyBrandStartupAssets(root, packaged, id);
    const foreignBrand = await loadBranding(root, id === "piora" ? "xiaoyi-harness" : "piora");
    await writeFile(resolve(packaged, "brand.json"), JSON.stringify(runtimeBranding(foreignBrand)));
    await assert.rejects(verifyBrandStartupAssets(root, packaged, id), /brand configuration mismatch/);
    await cp(resolve(root, ".branding/runtime.json"), resolve(packaged, "brand.json"));
    await verifyBrandStartupAssets(root, packaged, id);
    const icon = resolve(packaged, "startup/icon.png");
    const bytes = await readFile(icon);
    await writeFile(icon, "wrong brand icon");
    await assert.rejects(verifyBrandStartupAssets(root, packaged, id), /differs from brand source/);
    await rm(icon);
    await assert.rejects(verifyBrandStartupAssets(root, packaged, id), /missing or unexpected/);
    await writeFile(icon, bytes);
    if (id === "xiaoyi-harness") {
      await cp(resolve(root, "desktop/build/startup/polaris-rover.mp4"), resolve(packaged, "startup/polaris-rover.mp4"));
      await assert.rejects(verifyBrandStartupAssets(root, packaged, id), /missing or unexpected/);
    }
  }
});
...
test("custom tray PNG drives every Windows ICO size while Piora retains its default icon", async t => {
  const root = await fixture(t);
  await prepareBranding(root, "xiaoyi-harness");
  const source = resolve(root, "branding/xiaoyi-harness/tray.png");
  assert.deepEqual(await sharp(source).raw().toBuffer(), await sharp(resolve(root, "branding/xiaoyi-harness/tray.svg")).raw().toBuffer());
  ...
  for (const suffix of ["", "-preview"]) {
    const builder = JSON.parse(await readFile(resolve(root, `.branding/builder${suffix}.json`), "utf8"));
    assert.equal(builder.extraResources.find(entry => entry.to === "tray-icon.ico").from, "../.branding/resources/tray.ico");
  }
  await prepareBranding(root, "piora");
