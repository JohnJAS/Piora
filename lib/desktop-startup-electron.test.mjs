...
test("Windows Electron hands off intros and closes startup/application windows without native exceptions", {
  ...
  for (const name of ["branding", "generated/brand"]) {
    await writeFile(join(root, `${name}.js`), compile(await readFile(new URL(`../desktop/src/${name}.ts`, import.meta.url), "utf8")));
  }
  const probe = async function (startupSource, applicationSource) {
    ...
    try {
      for (const scenario of ["invalid-media", "completed-intro", "repeated-skip", "legacy-navigation", "stalled-file", "unwritable-file", "close-before-ready", "close-on-continue", "xiaoyi-first-use", "xiaoyi-repeat"]) {
        ...
        const bindings = {
          ...
          APP_BRAND: { id: scenario.startsWith("xiaoyi-") ? "xiaoyi-harness" : "piora" },
          hasShownXiaoyiStartup: () => scenario === "xiaoyi-repeat", markXiaoyiStartupShown: () => { firstUseMarks++; },
          writeFileSync: (...args) => { if (scenario === "unwritable-file") throw new Error("EACCES startup.html"); writeFileSync(...args); },
          loadStartupMedia: () => scenario === "repeated-skip" || scenario.startsWith("xiaoyi-") ? {} : { video: "data:video/mp4;base64,AA==" },
        ...
        const methods = new Function(...Object.keys(bindings), startupSource + applicationSource + ";return { createStartupWindow, loadApplicationWindow };")(...Object.values(bindings));
        {
          ...
          if (scenario === "xiaoyi-first-use") {
            assert.ok(Date.now() - await startup.ready >= 5000, "real Electron first-use handoff waits five seconds after readiness");
            assert.equal(firstUseMarks, 1);
          } else assert.equal(firstUseMarks, 0, "other scenarios cannot consume XiaoYi first use");
