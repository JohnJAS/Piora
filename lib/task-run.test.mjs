...
test("projects waiting Goal state into the shared user-input phase", () => {
  ...
  assert.equal(taskRun?.source, "goal");
  assert.equal(taskRun?.phase, "waiting_user");
  assert.equal(taskRun?.taskId, "goal-1");
});
