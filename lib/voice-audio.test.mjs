...
test("combines, resamples, and encodes microphone audio for the local runtime", () => {
...
test("rejects audio that does not match the local PCM input format", () => {
...
test("rejects silent recordings before local model inference", () => {
  assert.equal(containsAudibleSpeech(new Float32Array(16_000), 16_000), false);
  const speech = new Float32Array(16_000);
  speech.fill(0.02, 1_000, 2_500);
  assert.equal(containsAudibleSpeech(speech, 16_000), true);
});
