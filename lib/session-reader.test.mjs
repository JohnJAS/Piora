...
const {
  ...
  readSessionHeader,
  readPersistedGoalFromTail,
  resolveSessionIdByPath,
} = await jiti.import("./session-reader.ts");
const { GOAL_RUN_ENTRY_TYPE, GOAL_RUN_SCHEMA_VERSION } = await jiti.import("./goal-run-registry.ts");
const { getDisplayableAssistantBlocks, splitFinalAssistantBlocks } = await jiti.import("./message-display.ts");
...
test("resolves active settings outside the compaction projection", () => {
  const entries = [
    {
    ...
    userEntry("u1", "model", "old request"),
    {
      type: "compaction",
      id: "cmp",
      parentId: "u1",
  ...
  assert.deepEqual(context.entryIds, ["cmp", "u1", "u2"]);
});

test("resolves settings from the selected branch only", () => {
  const entries = [
    ...
    userEntry("main", "main-model", "main branch"),
    {
      type: "thinking_level_change",
      id: "alt-thinking",
      parentId: "root",
    ...
    {
      ...assistantEntry("alt", "alt-thinking", "alternate branch"),
      message: {
        role: "assistant",
  ...
  assert.deepEqual(context.entryIds, ["root", "alt"]);
});
...
test("preserves the raw block index when an earlier thinking block is filtered", () => {
  const entries = [
    userEntry("u1", null, "start"),
    {
      ...assistantEntry("a1", "u1", "answer"),
      message: {
        ...
        content: [
          { type: "thinking", thinking: "" },
          { type: "thinking", thinking: "reasoning at raw index one" },
          { type: "text", text: "answer" },
        ],
      },
    },
  ...
  assert.equal(assistantMessage.content[1].deferredBlockIndex, 1);

  const displayable = getDisplayableAssistantBlocks(assistantMessage);
  assert.equal(displayable[0].type, "thinking");
  assert.equal(displayable[0].deferredBlockIndex, 1);

  const split = splitFinalAssistantBlocks(assistantMessage);
  assert.equal(split.processBlocks[0].type, "thinking");
  assert.equal(split.processBlocks[0].deferredBlockIndex, 1);
});
...
test("reads the latest persisted goal from a bounded session-file tail", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-goal-tail-"));
  const filePath = join(dir, "session.jsonl");
  const sessionId = "goal-tail-session";
  const goal = {
    schemaVersion: GOAL_RUN_SCHEMA_VERSION,
    sessionId,
    runId: "run-tail",
    goalId: "goal-tail",
    objective: "Persist sidebar state",
    successCriteria: ["Visible after cold start"],
    constraints: [],
    status: "blocked",
    iteration: 2,
    startedAt: 1,
    ...
    checkpoints: [],
    evidence: [],
    reason: "Waiting for an external service",
  };
  const entry = { type: "custom", id: "entry", parentId: null, timestamp: new Date().toISOString(), customType: GOAL_RUN_ENTRY_TYPE, data: goal };
  writeFileSync(filePath, `${"x".repeat(300 * 1024)}\n${JSON.stringify(entry)}\n`, "utf8");
  try {
    assert.deepEqual(readPersistedGoalFromTail(filePath, sessionId), goal);
    assert.equal(readPersistedGoalFromTail(filePath, "other-session"), undefined);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
