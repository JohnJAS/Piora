...
test("flow analysis expands prototype-reachable pages without changing ordinary selections", async (t) => {
  const projectRoot = createHarmonyProject(t);
  const imported = record(projectRoot);
  const adapter = { async getNodes(_source, ids) { return payloads(ids); } };
  const selection = await analyzeDesignSelection({ record: imported, targetNodeIds: ["10:1"], adapter, project: analyzeHarmonyProject(projectRoot) });
  const flow = await analyzeDesignSelection({ record: imported, targetNodeIds: ["10:1"], adapter, project: analyzeHarmonyProject(projectRoot), includeInteractionTargets: true });
  assert.deepEqual(selection.plan.targetNodeIds, ["10:1"]);
  assert.deepEqual(flow.plan.targetNodeIds, ["10:1", "10:2"]);
  assert.equal(selection.plan.files.length, 1);
  assert.equal(flow.plan.files.length, 2);
  assert.notEqual(
    designAnalysisRunId(projectRoot, imported.id, imported.document.version.id, ["10:1"]),
    designAnalysisRunId(projectRoot, imported.id, imported.document.version.id, ["10:1"], "flow"),
  );
});
...
test("analysis routes are bounded, versioned, cacheable, and project scoped", () => {
  ...
  assert.match(route, /createDesignSourceAdapter/);
