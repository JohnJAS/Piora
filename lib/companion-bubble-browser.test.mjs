...
test("independent companion bubble follows session snapshots inside the native window bounds", { timeout: 120000 }, async () => {
  ...
  try {
    ...
    const focusTimer = { phase: "focus", status: "running", durations: { focus: 1_500, "short-break": 300, "long-break": 900 }, longBreakEvery: 4, autoStartNextPhase: false, petReminderEnabled: true, durationSeconds: 1_500, remainingSeconds: 1_500, startedAt: Date.now(), endsAt: Date.now() + 1_500_000, linkedTodoId: null, completedFocusSessions: 0 };
    ...
    await page.route("http://companion-bubble.test/**", (route) => {
      ...
      if (url.pathname === "/api/companion/state") return route.fulfill({ json: { updatedAt: 1, focusTimer, mind: { lastDecision: null } } });
    ...
    const bubbles = page.getByTestId("companion-activity-bubble");
    const stack = page.getByTestId("companion-status-stack");
    await bubbles.first().waitFor();
    await page.getByTestId("companion-focus-timer-bubble").waitFor();
    assert.match(await stack.innerText(), /专注中/);
    assert.match(await stack.innerText(), /正在修复宠物消息弹框/);
    assert.match(await stack.innerText(), /正在读取组件/);
    ...
    await page.evaluate((tasks) => window.publishTasks(tasks), [longTask, { ...task, id: "session-2", title: "第二个任务" }, { ...task, id: "session-3", title: "第三个任务" }]);
    await page.waitForFunction(() => document.querySelectorAll('[data-testid="companion-activity-bubble"]').length === 2 && document.querySelector('[data-testid="companion-status-stack"]')?.textContent.includes("1–2 / 3"));
    assert.match(await stack.innerText(), /第二个任务/);
    ...
    await page.waitForFunction(() => document.querySelector('[data-session-id="session-3"]'));
    ...
    await bubbles.first().waitFor({ state: "detached" });
    await page.getByTestId("companion-focus-timer-bubble").waitFor();
    await page.clock.runFor(200);
    await page.waitForFunction(() => getComputedStyle(document.querySelector("main")).opacity === "1");
