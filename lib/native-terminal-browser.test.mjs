...
test("native PowerShell preserves interactive editing, history predictions, sessions and application input", { skip: process.platform !== "win32" || process.env.PIORA_SKIP_RESOURCE_TESTS === "1", timeout: 180000 }, async () => {
  const root = await mkdtemp(path.join(tmpdir(), "piora-native-terminal-"));
  ...
  try {
    ...
    const run = async command => {
      ...
      await page.keyboard.press("Enter");
      await until(screen, text => text.includes("DONE_" + marker));
    };
    await run("$NativeValue = 41; Set-Location child");
    await run("Write-Output ('STATE=' + ($NativeValue + 1) + ':' + (Get-Location).Path)");
    assert.ok((await screen()).includes("STATE=42:" + path.join(root, "child")));
    ...
    assert.ok(path.resolve(root).startsWith(path.resolve(tmpdir()) + path.sep) && path.basename(root).startsWith("piora-native-terminal-"));
