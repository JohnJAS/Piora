...
function loadTools() {
  ...
  assert.deepEqual([...tools.keys()].sort(), ["harmony_control", "piora_harmony_check"]);
...
test("each Harmony entry stays below 600 estimated tokens", () => {
  const tools = loadTools();
  for (const name of ["harmony_control", "piora_harmony_check"]) {
    const tool = tools.get(name);
    assert.ok(tool);
    assert.ok(estimateToolDefinitionPromptTokens([tool]) < Math.min(600, TOOL_DEFINITION_PROMPT_TOKEN_LIMIT), `${name} stays within budget`);
  }
});
