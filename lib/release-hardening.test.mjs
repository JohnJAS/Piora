...
test("release workflows use immutable actions and gate the packaged runtime before publication", async () => {
  ...
  assert.match(desktopPackage.scripts.pack, /run-electron-builder\.mjs --dir --win --x64$/);
  assert.match(desktopPackage.scripts.dist, /run-electron-builder\.mjs --win --x64$/);
  assert.match(desktopPackage.scripts["dist:linux"], /run-electron-builder\.mjs --linux AppImage --x64$/);
  const builderRunner = await readFile(join(repositoryRoot, "scripts", "run-electron-builder.mjs"), "utf8");
  assert.match(builderRunner, /"--publish", "never"/);
  assert.match(builderRunner, /await verifyBrandBuilds\(projectRoot\)/);
  ...
  assert.match(desktopMain, /\(PORTABLE_SMOKE_TEST && !VERIFY_SMOKE_SINGLE_INSTANCE\) \|\| app\.requestSingleInstanceLock\(\)/);
  assert.match(
    desktopMain,
