...
export const coreFeatures = [
  {
    ...
    items: ['流式对话、代码高亮、Mermaid 图表与工具结果展示', '日常对话，以及可在设置中启用的 Goals / Plans 扩展', '任务运行中可随时引导（Steer）或排队（Queue）', '上下文用量可视，自动压缩不丢关键信息'],
  },
  {
    icon: '📂',
    tint: 'tint-cyan',
...
export const featureGroups = [
  {
    ...
    items: [
      ...
      '日常对话；Goals / Plans 作为可选第一方扩展，默认关闭',
  ...
  },
  {
    ...
    number: '10',
    title: '计划任务与持续跟进',
    summary: '提醒、监控、定期检查和项目作业交给 XiaoYiHarness 自己的持久化调度器。',
    items: [
...
export const quickStartSteps = [
  ...
  { title: '描述结果，而不是步骤', text: '例如：「检查登录流程，修复发现的问题，跑完测试并总结改动。」过程交给 XiaoYiHarness。' },
...
export const promptRecipes = [
  ...
  ['规划复杂工作', '先分析需求，列出有依赖关系的步骤、风险和验收标准，先不要改文件。'],
  ['跟进目标', '先梳理这项任务的目标和验收标准，再实施可完成的步骤，记录当前进度和下一步。'],
  ['组建团队', '创建一个团队：规划者拆解任务，执行者并行实现，审查者独立验收，最后统一汇总。'],
] as const;

export const faqs = [
  ['XiaoYiHarness 是什么？', 'XiaoYiHarness 是 Pi 的开源桌面工作台。它复用 Pi 的 AgentSession、模型、工具、skills 与 extensions，并补充现代桌面界面、文件和 Git 工作区、多人协作、设备自动化与本地安全边界。'],
  ['它和 Pi、pi-web、OpenAI 或 Codex 是什么关系？', 'XiaoYiHarness 是 Piora 的定制品牌版本，基于 pi-web 演进并面向 Pi 运行时，由社区独立维护；它不隶属于或代表 Pi、pi-web、OpenAI 或 Codex。'],
  ['必须会编程吗？', '不必。你可以先从 projectless Chat 开始普通问答；需要处理项目时再选择文件夹。页面上的五步指南和可复制提示词覆盖了第一次使用。'],
  ['我的代码和图片会上传吗？', 'XiaoYiHarness 没有分析 SDK、广告、账号服务或默认遥测。项目文件和自定义背景默认留在本机；模型请求、OAuth、第三方扩展和你主动选择的视觉模型会按相应提供商规则联网。'],
  ['免费吗？', '免费。XiaoYiHarness 本身以 MIT 协议开源，不收费、不内置会员；你只需要为自己的模型 API 用量向提供商付费。'],
