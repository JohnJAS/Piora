...
const SERVER_SNAPSHOT = `${DEFAULT_FONT_PREFERENCE.family}:${DEFAULT_FONT_PREFERENCE.size}`;
...
function getSnapshot(): string {
  ...
  return `${preference.family}:${preference.size}`;
}
...
export function useFontPreferences() {
  ...
  const [familyPart, sizePart] = snapshot.split(":");
