...
- 同步 upstream 的 beta.55 Git 与 SSH 更新，保留分品牌构建和更新行为，并刷新对应锁文件的许可证清单。
...
- 新增品牌切换资源回归、配置安全校验和双品牌发布汇总测试；当前 XiaoYiHarness 图标为开发示例，尚非确认的正式品牌素材。
- 打包前核对 Web 与桌面端的品牌编译记录，拒绝中途切换品牌或资源后复用旧编译产物，避免界面与更新器使用不同品牌。
- 安装包运行时冒烟检查增加窗口品牌与实际数据目录校验；新增仅限托管 Windows Actions 的连续安装升级脚本，使用隔离资料目录检查旧会话和设置文件保留，实际安装验证尚待执行。
