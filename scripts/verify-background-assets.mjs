...
if (
  ...
  || JSON.stringify(manifest.provenance?.records)
    !== JSON.stringify(["batch-a-generation.md", "batch-b-generation.md"])
) {
  failures.push("manifest provenance must reference the committed generation records");
}
if (!Array.isArray(manifest.presets) || manifest.presets.length !== 20) {
  failures.push("manifest must contain exactly 20 presets");
}
...
for (const recordFile of ["batch-a-generation.md", "batch-b-generation.md", "README.md"]) {
